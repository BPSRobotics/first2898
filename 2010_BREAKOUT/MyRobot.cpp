...
|__   __|                   |__ \ / _ \ / _ \ / _ \ 
   ...
   | |/ _ \/ _` | '_ ` _ \    / / > _ < \__, |> _ < 
   | |  __/ (_| | | | | | |  / /_| (_) |  / /| (_) |
   |_|\___|\__,_|_| |_| |_| |____|\___/  /_/  \___/ 
...
public:
	...
	//	m_base->ArcadeDrive(0.0, output); arcade drive won't work for our robot
		m_base->HolonomicDrive(0.0,0.0, output);
		outputValue = -output;
	}
private:
	RobotDrive *m_base;
...
class RobotDemo : public SimpleRobot {
	...
	Gyro *gyro;
	PIDOutput *pidOutput; //ask why this is pointer of inherited class...
...
public:
	RobotDemo(void) {
		...
		base = new RobotDrive(1, 2, 3, 4); //four motors for mechanum drive
	...
	/*Brian Gorman - I added in an autonomous period 
	 * right now it just drives forward*/
...
//						targets[0].Print();
				...
				base->HolonomicDrive(stick->GetDirectionDegrees(),stick->GetMagnitude(),stick->GetTwist());
				//maybe change rate of rotation to throttle
			}
			lastTrigger = trigger;
