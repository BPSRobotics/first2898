...
public:
	RobotDemo(void) {
		...
		base = new RobotDrive(4,5,3,7); //3,5,7,8 work
	...
	void Autonomous(void)
		{
			...
			base->Drive(0.5, 0.0); 	// drive forwards half speed
			Wait(2.0); 				//    for 2 seconds
			base->Drive(0.0, 0.0); 	// stop robot
		}
...
//						targets[0].Print();
			...
			else 
			{
				...
				if (trigger != lastTrigger)
					turnController.Disable();
				base->HolonomicDrive(stick->GetMagnitude(),stick->GetDirectionDegrees(),stick->GetTwist());
				kicker->Set(1.0);
				printf("I am setting the kicker!\n Hi steven\n");
			}
			lastTrigger = trigger;
